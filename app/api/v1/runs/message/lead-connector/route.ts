...
export async function POST(request: Request) {
  try {
    ...
    let messageContent = '';
    if (runMessageData && runMessageData.runMessage && runMessageData.runMessage.choices && runMessageData.runMessage.choices.length > 0 && runMessageData.runMessage.choices[0].message) {
    ...
    const apiResponse = await fetch('https://services.leadconnectorhq.com/conversations/messages', apiRequestOptions);
